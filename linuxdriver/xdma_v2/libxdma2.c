...
/* Module Parameters */
#ifdef XDMA2_CONFIG_BAR_NUM
static int config_bar_num=XDMA2_CONFIG_BAR_NUM;
#else 
...
#ifdef XDMA2_C2H_CREDITS
static unsigned int enable_st_c2h_credit = XDMA2_C2H_CREDITS;
#else
...
#ifdef XDMA2_H2C_TIMEOUT
unsigned int h2c_timeout_ms = XDMA2_H2C_TIMEOUT;
#else
...
#ifdef XDMA2_C2H_TIMEOUT
unsigned int c2h_timeout_ms = XDMA2_C2H_TIMEOUT;
#else 
...
/*
 * xdma2 device management
 * maintains a list of the xdma2 devices
 */
...
static inline int xdev_list_add(struct xdma2_dev *xdev)
{
	...
	if (list_empty(&xdev_list)) {
		...
		struct xdma2_dev *last;

		last = list_last_entry(&xdev_list, struct xdma2_dev, list_head);
		xdev->idx = last->idx + 1;
	}
	list_add_tail(&xdev->list_head, &xdev_list);
	mutex_unlock(&xdev_mutex);

	dbg_init("dev %s, xdev 0x%p, xdma2 idx %d.\n",
...
static inline void xdev_list_remove(struct xdma2_dev *xdev)
{
	mutex_lock(&xdev_mutex);
...
struct xdma2_dev *xdev_find_by_pdev(struct pci_dev *pdev)
{
	struct xdma2_dev *xdev, *tmp;
...
static void check_nonzero_interrupt_status(struct xdma2_dev *xdev)
{
	...
	if (w) {
		pr_info("%s xdma2%d user_int_enable = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
	w = read_register(&reg->channel_int_enable);
	if (w) {
		pr_info("%s xdma2%d channel_int_enable = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
	w = read_register(&reg->user_int_request);
	if (w) {
		pr_info("%s xdma2%d user_int_request = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
	w = read_register(&reg->channel_int_request);
	if (w) {
		pr_info("%s xdma2%d channel_int_request = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
	w = read_register(&reg->user_int_pending);
	if (w) {
		pr_info("%s xdma2%d user_int_pending = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
	w = read_register(&reg->channel_int_pending);
	if (w) {
		pr_info("%s xdma2%d channel_int_pending = 0x%08x\n",
			dev_name(&xdev->pdev->dev), xdev->idx, w);
	}
}

/* channel_interrupts_enable -- Enable interrupts we are interested in */
static void channel_interrupts_enable(struct xdma2_dev *xdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);

	write_register(mask, &reg->channel_int_enable_w1s, XDMA2_OFS_INT_CTRL);
}

/* channel_interrupts_disable -- Disable interrupts we not interested in */
static void channel_interrupts_disable(struct xdma2_dev *xdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);

	write_register(mask, &reg->channel_int_enable_w1c, XDMA2_OFS_INT_CTRL);
}

/* user_interrupts_enable -- Enable interrupts we are interested in */
static void user_interrupts_enable(struct xdma2_dev *xdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);

	write_register(mask, &reg->user_int_enable_w1s, XDMA2_OFS_INT_CTRL);
}

/* user_interrupts_disable -- Disable interrupts we not interested in */
static void user_interrupts_disable(struct xdma2_dev *xdev, u32 mask)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);

	write_register(mask, &reg->user_int_enable_w1c, XDMA2_OFS_INT_CTRL);
}

/* read_interrupts -- Print the interrupt controller status */
static u32 read_interrupts(struct xdma2_dev *xdev)
{
	struct interrupt_regs *reg =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);
...
void enable_perf(struct xdma2_engine *engine)
{
	u32 w;
	...
	w = XDMA2_PERF_AUTO | XDMA2_PERF_RUN;
	write_register(w, &engine->regs->perf_ctrl,
		       (unsigned long)(&engine->regs->perf_ctrl) -
			       (unsigned long)(&engine->regs));
	read_register(&engine->regs->identifier);

	dbg_perf("XDMA2_IOCTL_PERF_START\n");
}

void get_perf_stats(struct xdma2_engine *engine)
{
	u32 hi;
	u32 lo;
#ifdef __LIBXDMA2_DEBUG__
	...
	engine->xdma2_perf.clock_cycle_count = build_u64(hi, lo);

	hi = read_register(&engine->regs->perf_dat_hi);
	lo = read_register(&engine->regs->perf_dat_lo);
	engine->xdma2_perf.data_cycle_count = build_u64(hi, lo);
...
static bool engine_process_status(struct xdma2_engine *engine, u32 status)
{
	
	const u32 mask_good=(XDMA2_STAT_DESC_STOPPED|XDMA2_STAT_DESC_COMPLETED);
	const u32 mask_error=~(mask_good|XDMA2_STAT_BUSY|XDMA2_STAT_IDLE_STOPPED);

	dbg_tfr("XDMA2 engine %s status: 0x%08x \n", engine->name, status);
	
	/* check if an error caused the interrupt*/
	
	if(unlikely(status & mask_error))
	{
		pr_err("XDMA2 engine %s reports following error(s):\n", engine->name);
	
		if (status & XDMA2_STAT_ALIGN_MISMATCH)
			pr_err("Source and destination adress are not properly" 
			"aligned to each other\n");
		if (status & XDMA2_STAT_MAGIC_STOPPED)
			pr_err("A descriptor has invalid magic bit field\n");
		if (status & XDMA2_STAT_INVALID_LEN)
			pr_err("Length of a descriptor is not multiple of datapath width\n");

		if (engine->dir == DMA_TO_DEVICE) 
		{
			/* H2C only */
			if (status & XDMA2_STAT_H2C_R_ERR_MASK) 
			{
				pr_err("PCI-E read error(s): ");
				if (status & XDMA2_STAT_H2C_R_UNSUPP_REQ)
					pr_cont("unsupported request ");
				if (status & XDMA2_STAT_H2C_R_COMPL_ABORT)
					pr_cont("completer abort ");
				if (status & XDMA2_STAT_H2C_R_PARITY_ERR)
					pr_cont("parity error ");
				if (status & XDMA2_STAT_H2C_R_HEADER_EP)
					pr_cont("header EP ");
				if (status & XDMA2_STAT_H2C_R_UNEXP_COMPL)
			...
			if ((status & XDMA2_STAT_H2C_W_ERR_MASK)) 
			{
				pr_err("PCI-E write error: ");
				if (status & XDMA2_STAT_H2C_W_DECODE_ERR)
					pr_cont("decode error ");
				if (status & XDMA2_STAT_H2C_W_SLAVE_ERR)
					pr_cont("slave error ");
		...
		{
			/* C2H only */
			if ((status & XDMA2_STAT_C2H_R_ERR_MASK)) 
			{
				pr_err("PCI-E read error(s): ");
			
				if (status & XDMA2_STAT_C2H_R_DECODE_ERR)
					pr_cont("decode error ");
				if (status & XDMA2_STAT_C2H_R_SLAVE_ERR)
					pr_cont("slave error ");
		...
		/* common H2C & C2H */
		if ((status & XDMA2_STAT_DESC_ERR_MASK)) 
		{
			pr_err("Descriptor error(s): ");
			if (status & XDMA2_STAT_DESC_UNSUPP_REQ)
				pr_cont("unsupported request ");
			if (status & XDMA2_STAT_DESC_COMPL_ABORT)
				pr_cont("completer abort ");
			if (status & XDMA2_STAT_DESC_PARITY_ERR)
				pr_cont("parity error ");
			if (status & XDMA2_STAT_DESC_HEADER_EP)
				pr_cont("header EP ");
			if (status & XDMA2_STAT_DESC_UNEXP_COMPL)
				pr_cont("unexpected completeion ");
...
static int xdma2_engine_stop(struct xdma2_engine *engine)
{
	u32 w=0x1;
...
static irqreturn_t user_irq_service(int irq, struct xdma2_user_irq *user_irq)
{
	unsigned long flags;

	xdma2_debug_assert_msg(user_irq!=NULL,"Invalid user_irq\n", IRQ_NONE);
...
/*
 * xdma2_isr() - Interrupt handler
 *
 * @dev_id pointer to xdma2_dev
 */
static irqreturn_t xdma2_isr(int irq, void *dev_id)
{
	u32 ch_irq;
	u32 user_irq;
	u32 mask;
	...
	xdev = (struct xdma2_dev *)dev_id;

	xdma2_debug_assert_msg(xdev!=NULL, "Invalid XDMA2 device\n", IRQ_NONE);
	...
	if (mask) {
		...
		for (channel = 0; channel < max && mask; channel++) {
			struct xdma2_engine *engine = &xdev->engine_h2c[channel];
	...
	if (mask) {
		...
		for (channel = 0; channel < max && mask; channel++) {
			struct xdma2_engine *engine = &xdev->engine_c2h[channel];
...
/*
 * xdma2_user_irq() - Interrupt handler for user interrupts in MSI-X mode
 *
 * @dev_id pointer to xdma2_dev
 */
static irqreturn_t xdma2_user_irq(int irq, void *dev_id)
{
	struct xdma2_user_irq *user_irq;
	...
	user_irq = (struct xdma2_user_irq *)dev_id;

	return user_irq_service(irq, user_irq);
}

/*
 * xdma2_channel_irq() - Interrupt handler for channel interrupts in MSI-X mode
 *
 * @dev_id pointer to xdma2_dev
 */
static irqreturn_t xdma2_channel_irq(int irq, void *dev_id)
{
	struct xdma2_dev *xdev;
	...
	engine = (struct xdma2_engine *)dev_id;
	xdev = engine->xdev;
 ...
 */
static void unmap_bars(struct xdma2_dev *xdev, struct pci_dev *dev)
{
	int i;

	for (i = 0; i < XDMA2_BAR_NUM; i++) {
		/* is this BAR mapped? */
		if (xdev->bar[i]) {
			/* unmap BAR */
...
static int map_single_bar(struct xdma2_dev *xdev, struct pci_dev *dev, int idx, resource_size_t *bar_len)
{
...
static bool is_config_bar(struct xdma2_dev *xdev, int idx, resource_size_t len)
{
	u32 irq_id = 0;
	u32 cfg_id = 0;
	u32 mask = 0xffff0000; /* Compare only XDMA2 ID's not Version number */
	
	struct interrupt_regs *irq_regs =
		(struct interrupt_regs *)(xdev->bar[idx] + XDMA2_OFS_INT_CTRL);
	struct config_regs *cfg_regs =
		(struct config_regs *)(xdev->bar[idx] + XDMA2_OFS_CONFIG);
		
	if (len<XDMA2_BAR_SIZE)
	{
		dbg_init("BAR %d is NOT the XDMA2 config BAR: 0x%x, 0x%x, len=%llu\n",
	...
	if (((irq_id & mask) == IRQ_BLOCK_ID) &&
		...
		dbg_init("BAR %d is the XDMA2 config BAR\n", idx);
		return true;
	} else {
		dbg_init("BAR %d is NOT the XDMA2 config BAR: 0x%x, 0x%x, len=%llu\n",
			 idx, irq_id, cfg_id, len);
...
static int identify_bars(struct xdma2_dev *xdev, int *bar_id_list, int num_bars,
			 int config_bar_pos)
{
	/*
	 * The following logic identifies which BARs contain what functionality
	 * based on the position of the XDMA2 config BAR and the number of BARs
	 * detected. The rules are that the user logic and bypass logic BARs
	 * are optional.  When both are present, the XDMA2 config BAR will be the
	 ...
	 * XDMA2 config BAR is detected first or last.  When both are omitted,
	 * only the XDMA2 config BAR is present.  This somewhat convoluted
	 * approach is used instead of relying on BAR numbers in order to work
	 * correctly with both 32-bit and 64-bit BARs.
	 */

	xdma2_debug_assert_ptr(xdev);

	xdma2_debug_assert_ptr(bar_id_list);
	...
	case 2:
		...
		if (config_bar_pos == 0) {
			...
			pr_warn("2 BARs, XDMA2 config BAR unexpected %d.\n",
				config_bar_pos);
		}
		break;
	...
	case 4:
		...
		if ((config_bar_pos == 1) || (config_bar_pos == 2)) {
			...
			pr_warn("3/4 BARs, XDMA2 config BAR unexpected %d.\n",
				config_bar_pos);
		}
		break;
	...
	default:
		...
		pr_warn("Unexpected # BARs (%d), XDMA2 config BAR only.\n",
			num_bars);
		break;
	}
...
no_config_bar:
	pr_err("Failed to detect XDMA2 config BAR\n");
	return -ENODEV;
}
 ...
 */
static int map_bars(struct xdma2_dev *xdev, struct pci_dev *dev)
{
	int rv;

	int i;
	int bar_id_list[XDMA2_BAR_NUM];
	int bar_id_idx = 0;
...
static void disable_msi_msix(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
...
static int enable_msi_msix(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
	int rv = 0;
...
static void prog_irq_msix_user(struct xdma2_dev *xdev, bool clear)
{
	/* user */
...
static void prog_irq_msix_channel(struct xdma2_dev *xdev, bool clear)
{
	struct interrupt_regs *int_regs =
		(struct interrupt_regs *)(xdev->bar[xdev->config_bar_idx] +
					  XDMA2_OFS_INT_CTRL);
...
static void irq_msix_channel_teardown(struct xdma2_dev *xdev)
{
	struct xdma2_engine *engine;
	int j = 0;
	int i = 0;
...
static int irq_msix_channel_setup(struct xdma2_dev *xdev)
{
	int i;
	int j;
	int rv = 0;
	...
	for (i = 0; i < xdev->h2c_channel_num; i++, engine++) {
		...
		rv = request_irq(vector, xdma2_channel_irq, 0, xdev->mod_name,
				 engine);
		if (rv) {
...
static void irq_msix_user_teardown(struct xdma2_dev *xdev)
{
	int i;
	int j;
#ifdef __LIBXDMA2_DEBUG__
...
static int irq_msix_user_setup(struct xdma2_dev *xdev)
{
	int i;
	...
	for (i = 0; i < xdev->user_max; i++, j++) {
		...
		rv = request_irq(vector, xdma2_user_irq, 0, xdev->mod_name,
				 &xdev->user_irq[i]);
		if (rv) {
...
static int irq_msi_setup(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
	int rv;

	xdev->irq_line = (int)pdev->irq;
	rv = request_irq(pdev->irq, xdma2_isr, 0, xdev->mod_name, xdev);
	if (rv)
...
static int irq_legacy_setup(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
	u32 w;
	u8 val;
	void *reg;
	...
	xdev->irq_line = (int)pdev->irq;
	rv = request_irq(pdev->irq, xdma2_isr, IRQF_SHARED, xdev->mod_name,
			 xdev);
	if (rv)
		dbg_init("Couldn't use IRQ#%d, %d\n", pdev->irq, rv);
	else
...
static void irq_teardown(struct xdma2_dev *xdev)
{
	if (xdev->msix_enabled) {
...
static int irq_setup(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
...
#ifdef __LIBXDMA2_DEBUG__
static void dump_desc(const struct xdma2_desc *desc_virt)
{
	int j;
	u32 *p = (u32 *)desc_virt;
...
#else 
#define dump_desc(...)  
#endif /* __LIBXDMA2_DEBUG__ */


static void engine_alignments(struct xdma2_engine *engine)
{
	u32 w;
	u32 align_bytes;
...
static void engine_free_resource(struct xdma2_engine *engine)
{
	dma_pool_destroy(engine->desc_pool);
	 
#ifdef XDMA2_POLL_MODE
...
static int engine_destroy(struct xdma2_dev *xdev, struct xdma2_engine *engine)
{
	xdma2_debug_assert_ptr(xdev);
	...
	memset(engine, 0, sizeof(struct xdma2_engine));
 ...
 */
static int engine_init_regs(struct xdma2_engine *engine)
{
	/*configure both control and interrupt registers just once*/
	u32 interrupt_reg_value=0;
	u32 control_reg_value=
			(XDMA2_CTRL_IE_DESC_ERROR
			|XDMA2_CTRL_IE_READ_ERROR
			|XDMA2_CTRL_IE_WRITE_ERROR
			|XDMA2_CTRL_IE_INVALID_LENGTH
			|XDMA2_CTRL_IE_MAGIC_STOPPED
			|XDMA2_CTRL_IE_DESC_ALIGN_MISMATCH
			|XDMA2_CTRL_IE_DESC_COMPLETED
			|XDMA2_CTRL_IE_DESC_STOPPED);
...
/* if using polled mode,  enable writeback and configure its address, disable interrupts */
	...
	control_reg_value|=XDMA2_CTRL_POLL_MODE_WB;
#else
	...
	control_reg_value|=XDMA2_CTRL_STM_MODE_WB;
	...
	if (enable_st_c2h_credit && engine->streaming &&
	    engine->dir == DMA_FROM_DEVICE) {
		struct xdma2_dev *xdev = engine->xdev;
		u32 reg_value = (0x1 << engine->channel) << 16;
		struct sgdma_common_regs *reg =
			(struct sgdma_common_regs
				 *)(xdev->bar[xdev->config_bar_idx] +
...
static int engine_alloc_resource(struct xdma2_engine *engine)
{
	struct xdma2_dev *xdev = engine->xdev;
	...
	engine->desc_pool= dma_pool_create(strncat(pool_name, engine->name, sizeof(pool_name)-20-1), &(xdev->pdev->dev),
                engine->adj_block_len * sizeof(struct xdma2_desc), sizeof(struct xdma2_desc), XDMA2_PAGE_SIZE);
...
#ifdef XDMA2_POLL_MODE
	engine->poll_mode_wb.virtual_addr=dma_alloc_coherent( &(xdev->pdev->dev), sizeof(struct xdma2_poll_wb), 
	...
	engine->poll_mode_wb.length= sizeof(struct xdma2_poll_wb);
...
static int engine_init(struct xdma2_dev *xdev, enum dma_data_direction dir, int channel)
{
	int rv;
	u32 val;
	unsigned int offset=get_engine_offset(dir, channel);
	struct xdma2_engine *engine= dir==DMA_TO_DEVICE? &(xdev->engine_h2c[channel]): &(xdev->engine_c2h[channel]);
...
#ifndef XDMA2_POLL_MODE
	/* engine interrupt request bit */
	engine->irq_bitmask = (1 << XDMA2_ENG_IRQ_NUM) - 1;
	engine->irq_bitmask <<= (xdev->engines_num * XDMA2_ENG_IRQ_NUM);
#endif
	/* parent */
	engine->xdev = xdev;
	/* register address */
	...
	const_cast(unsigned int, engine->desc_max) = (XDMA2_DESC_FIFO_DEPTH * xdev->datapath_width) /sizeof(struct xdma2_desc)
	    				/(dir==DMA_TO_DEVICE? xdev->h2c_channel_num: xdev->c2h_channel_num);
	
	    	
	const_cast(unsigned int, engine->adj_block_len)=engine->xdev->max_read_request_size /sizeof(struct xdma2_desc);
	...
	init_completion(&(engine->engine_compl));
	#endif
	/*pr_info("XDMA2 engine %s can use up to %u descriptors with length of block of adjacent descriptors up to %u", 
...
static int xdma2_validate_transfer(const struct xdma2_engine *engine)
{	
	int rv=0;
	const struct xdma2_transfer_params *transfer_params=&(engine->transfer_params);
...
static int xdma2_sgtable_to_descriptors(struct xdma2_engine *engine)
{
	struct xdma2_transfer *transfer=&(engine->transfer);
	...
	u32 control_flags=(XDMA2_DESC_STOPPED|XDMA2_DESC_COMPLETED);
	
	if(engine->streaming&& (engine->dir==DMA_TO_DEVICE) && engine->eop_flush)
		control_flags|=XDMA2_DESC_EOP;	
	...
	for(; (block_num<transfer->num_adj_blocks) && (processed_sg_entries < sg_nents); ++block_num)
	{
		...
		struct xdma2_desc *current_desc;
		...
		transfer->adj_desc_blocks[block_num].length=engine->adj_block_len*sizeof(struct xdma2_desc);
		...
		for(; (processed_sg_entries < sg_nents) && (i<engine->adj_block_len); ++i,  desc_dma_addr+=sizeof(struct xdma2_desc))
		{
			...
			while((processed_sg_entries < sg_nents))
			{
				...
				if(likely(desc_length_accum<=XDMA2_DESC_BLEN_MAX))
				{
...
cause quite a mess*/ 
static int xdma2_prepare_transfer(struct xdma2_engine *engine)
{
	int rv=0;
	const struct xdma2_transfer_params *transfer_params=&(engine->transfer_params);
	struct xdma2_transfer *transfer=&(engine->transfer);
	...
	rv=sg_alloc_table_from_pages_segment(&(transfer->sgt), transfer->pages, 
			...
			transfer_params->length, XDMA2_DESC_BLEN_MAX, GFP_KERNEL|__GFP_NORETRY);
	...
	rv=xdma2_sgtable_to_descriptors(engine);
...
static inline u32 get_initial_adj_count(struct xdma2_engine *engine, unsigned int adj_block_num)
{
	return (le32_to_cpu(engine->transfer.adj_desc_blocks[adj_block_num].virtual_addr[0].control) & DESC_ADJ_MASK)>>DESC_ADJ_SHIFT;
}

static void xdma2_launch_transfer(struct xdma2_engine *engine)
{
	...
	if((engine->dir==DMA_FROM_DEVICE)&&(engine->streaming)&&(enable_st_c2h_credit>0))
		write_register(min_t(unsigned int, enable_st_c2h_credit, XDMA2_MAX_C2H_CREDITS), 
	...
	write_register(XDMA2_CTRL_RUN_STOP, &(engine->regs->control_w1s), 0);
...
static long xdma2_wait_for_transfer(struct xdma2_engine *engine)
{
	...
	{	
		...
		/*return a positive value. xdma2_finalise_transfer will deal with it appropriately*/
...
static ssize_t calculate_completed_length(const struct xdma2_engine *engine, u32 num_descriptors)
{
...
static ssize_t xdma2_finalise_transfer(struct xdma2_engine *engine, ssize_t transfer_result)
{
#ifndef XDMA2_POLL_MODE
...
static void xdma2_cleanup_transfer(struct xdma2_engine *engine, bool transfer_ok)
{
	struct xdma2_transfer *transfer=&(engine->transfer);
	...
	memset(transfer, 0, sizeof(struct xdma2_transfer));
	
}


ssize_t xdma2_xfer_submit(struct xdma2_engine *engine)
{

	ssize_t rv=0;
	rv=xdma2_validate_transfer(engine);
	if (rv<0)
		return rv; 
		
	rv=xdma2_prepare_transfer(engine);
	if(rv<0)
		goto cleanup;
	xdma2_launch_transfer(engine);
	rv=xdma2_wait_for_transfer(engine);
	rv=xdma2_finalise_transfer(engine, rv);
	
	cleanup:
	xdma2_cleanup_transfer(engine, rv>0);
	
	return rv;
}


int xdma2_performance_submit(struct xdma2_engine *engine)
{
	u64 ep_addr = engine->streaming? : engine->xdma2_perf.axi_address;
	int i=0;
	int rv = 0;
	struct xdma2_desc *current_desc;
	/*array of DMA records for performance buffers*/
	generic_dma_record(u8) *perf_bufs=NULL;
	unsigned int num_bufs= divide_roundup(engine->xdma2_perf.transfer_size, KMALLOC_MAX_SIZE);
	unsigned int next_adj=num_bufs - 1;
	dma_addr_t desc_dma_addr;
	if(num_bufs > engine->adj_block_len)
		return -EFBIG;
		
	if(engine->xdma2_perf.transfer_size==0 ||
	engine->xdma2_perf.transfer_size & (engine->xdev->datapath_width-1))
	{
	...
	/*neccessary for correct function of xdma2_finalise_transfer*/
	engine->transfer_params.length=engine->xdma2_perf.transfer_size;
	perf_bufs=kcalloc(num_bufs, sizeof(*perf_bufs), GFP_KERNEL|__GFP_RETRY_MAYFAIL);
	if (perf_bufs==NULL)
		return -ENOMEM;
	/*in order to use xdma2_launch_transfer*/
	...
	engine->transfer.adj_desc_blocks->length=engine->adj_block_len*sizeof(struct xdma2_desc);
	
	for(; i<num_bufs; ++i, desc_dma_addr+=sizeof(struct xdma2_desc), --next_adj)
	{
		unsigned int buf_length= (i==(num_bufs-1))? engine->xdma2_perf.transfer_size & (KMALLOC_MAX_SIZE-1) : KMALLOC_MAX_SIZE;
	...
	current_desc->control|=(XDMA2_DESC_STOPPED|XDMA2_DESC_COMPLETED);
	dump_desc(current_desc);
	...
	rv=xdma2_wait_for_transfer(engine);
	rv=xdma2_finalise_transfer(engine, rv);
	if(rv!=engine->xdma2_perf.transfer_size)
	{
...
static struct xdma2_dev *alloc_dev_instance(struct pci_dev *pdev)
{
	int i;
	struct xdma2_dev *xdev;
	
	xdma2_debug_assert_msg(pdev!=NULL, "Invalid pdev\n", NULL);

	/* allocate zeroed device book keeping structure */
	xdev = kzalloc(sizeof(struct xdma2_dev), GFP_KERNEL);
	if (!xdev) {
		pr_info("OOM, xdma2_dev.\n");
		return NULL;
	}
...
static int request_regions(struct xdma2_dev *xdev, struct pci_dev *pdev)
{
	int rv;
...
static int set_dma_mask(struct pci_dev *pdev)
{
	...
	/* 64-bit addressing capability for XDMA2? avilible since 3.13*/
...
static void remove_engines(struct xdma2_dev *xdev)
{
	struct xdma2_engine *engine;
	int i;
	int rv;
...
static inline void set_max_read_request_size(struct xdma2_dev *xdev)
{
	unsigned int max_read_request_size=128;
	struct config_regs *cfg_regs= (struct config_regs *)(xdev->bar[xdev->config_bar_idx] + XDMA2_OFS_CONFIG);	
...
static inline void set_datapath_width(struct xdma2_dev *xdev)
{
...
static bool probe_for_engine(struct xdma2_dev *xdev, enum dma_data_direction dir,
			    int channel)
{
	...
	if (dir == DMA_TO_DEVICE) {
		engine_id_expected = XDMA2_ID_H2C;
		engine = &xdev->engine_h2c[channel];
	} else {
		engine_id_expected = XDMA2_ID_C2H;
...
static int probe_engines(struct xdma2_dev *xdev)
{
	int i=0;
	int rv = 0;
	...
	for(xdev->h2c_channel_num=0; (xdev->h2c_channel_num<XDMA2_CHANNEL_NUM_MAX)&&probe_for_engine(xdev, DMA_TO_DEVICE, xdev->h2c_channel_num); ++xdev->h2c_channel_num);
	...
	if(xdev->h2c_channel_num>0)
	{
		...
		xdev->engine_h2c=kcalloc(xdev->h2c_channel_num, sizeof(struct xdma2_engine), GFP_KERNEL|__GFP_RETRY_MAYFAIL);
	...
	for (xdev->c2h_channel_num = 0;(xdev->c2h_channel_num<XDMA2_CHANNEL_NUM_MAX)&&probe_for_engine(xdev, DMA_FROM_DEVICE, xdev->c2h_channel_num); ++xdev->c2h_channel_num);
	
	if (xdev->c2h_channel_num>0)
	{
		xdev->engine_c2h=kcalloc(xdev->c2h_channel_num, sizeof(struct xdma2_engine), GFP_KERNEL|__GFP_RETRY_MAYFAIL);
...
void *xdma2_device_open(const char *mname, struct pci_dev *pdev, int *user_max,
		       int *h2c_channel_num, int *c2h_channel_num)
{
	struct xdma2_dev *xdev = NULL;
	int rv = 0;
	...
	if (xdev->h2c_channel_num == 0 ||
	    xdev->h2c_channel_num > XDMA2_CHANNEL_NUM_MAX)
		xdev->h2c_channel_num = XDMA2_CHANNEL_NUM_MAX;
	if (xdev->c2h_channel_num == 0 ||
	    xdev->c2h_channel_num > XDMA2_CHANNEL_NUM_MAX)
		xdev->c2h_channel_num = XDMA2_CHANNEL_NUM_MAX;
	...
	dma_set_max_seg_size( &(pdev->dev), XDMA2_DESC_BLEN_MAX & PAGE_MASK);
	...
	dbg_init("XDMA2 MRRS is %u byte, datapath width is %u bit/%u byte", 
...
void xdma2_device_close(struct pci_dev *pdev, void *dev_hndl)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
...
void xdma2_device_offline(struct pci_dev *pdev, void *dev_hndl)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
	struct xdma2_engine *engine;
	int i;
...
void xdma2_device_online(struct pci_dev *pdev, void *dev_hndl)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
	struct xdma2_engine *engine;
	int i;
...
int xdma2_device_restart(struct pci_dev *pdev, void *dev_hndl)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
...
int xdma2_user_isr_register(void *dev_hndl, unsigned int mask,
...
int xdma2_user_isr_enable(void *dev_hndl, unsigned int mask)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
...
int xdma2_user_isr_disable(void *dev_hndl, unsigned int mask)
{
	struct xdma2_dev *xdev = (struct xdma2_dev *)dev_hndl;
...
void engine_addrmode_set(struct xdma2_engine *engine, bool set)
{
	u32 w = XDMA2_CTRL_NON_INCR_ADDR;
	dbg_perf("XDMA2_IOCTL_ADDRMODE_SET\n");
