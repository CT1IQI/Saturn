...
#ifdef XDMA2_HIERARCHICAL_DEVICES
	#define XDMA2_NAME_SEPARATOR "/"
	#ifndef XDMA2_PREFIX
		#define XDMA2_NODE_NAME	"xdma2/"
	#else 
		#define XDMA2_NODE_NAME XDMA2_PREFIX "/"
	#endif

#else
	#define XDMA2_NAME_SEPARATOR "_"
	#ifndef XDMA2_PREFIX
		#define XDMA2_NODE_NAME	"xdma2"
	#else 
		#define XDMA2_NODE_NAME XDMA2_PREFIX
...
static struct class *g_xdma2_class;


static const char * const devnode_names[] = {
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "user",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "control",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "xvc",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "events_%d",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "h2c_%d",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "c2h_%d",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "bypass_h2c_%d",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "bypass_c2h_%d",
	XDMA2_NODE_NAME "%d" XDMA2_NAME_SEPARATOR "bypass",
};
...
static inline void xpdev_flag_set(struct xdma2_pci_dev *xpdev,
				enum xpdev_flags_bits fbit)
{
	xpdev->flags |= 1 << fbit;
}

static inline void xcdev_flag_clear(struct xdma2_pci_dev *xpdev,
				enum xpdev_flags_bits fbit)
{
	xpdev->flags &= ~(1 << fbit);
}

static inline int xpdev_flag_test(struct xdma2_pci_dev *xpdev,
				enum xpdev_flags_bits fbit)
{
	return xpdev->flags & (1 << fbit);
}

#ifdef __XDMA2_SYSFS__
ssize_t xdma2_dev_instance_show(struct device *dev,
		struct device_attribute *attr,
		char *buf)
{
	struct xdma2_pci_dev *xpdev =
		(struct xdma2_pci_dev *)dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%d\t%d\n",
			xpdev->major, xpdev->xdev->idx);
}

static DEVICE_ATTR_RO(xdma2_dev_instance);
#endif

static int config_kobject(struct xdma2_cdev *xcdev, enum cdev_type type)
{
	int rv = -EINVAL;
	struct xdma2_dev *xdev = xcdev->xdev;
...
int xcdev_check(const char *fname, struct xdma2_cdev *xcdev, bool check_engine)
{
...
int char_open(struct inode *inode, struct file *filp)
{
	...
	xcdev = container_of(inode->i_cdev, struct xdma2_cdev, cdev);
	xdma2_debug_assert_msg(xcdev->magic == MAGIC_CHAR,"char magic mismatch\n",-EINVAL);
...
loff_t char_llseek(struct file *filp, loff_t off, int whence)
{
	...
	struct xdma2_dev *xdev = xcdev->xdev;
	loff_t newpos = 0;
	/*XDMA2 Address space is unlimited, while other interfaces are limited by their BAR sizes*/
	resource_size_t maxpos = ((xcdev->type==CHAR_XDMA2_H2C)||(xcdev->type==CHAR_XDMA2_C2H))? 
...
int char_close(struct inode *inode, struct file *filp)
{
	...
	xdma2_debug_assert_msg(xdev->magic == MAGIC_DEVICE,"device magic mismatch\n", -EINVAL);
...
static int create_sys_device(struct xdma2_cdev *xcdev, enum cdev_type type)
{
	struct xdma2_dev *xdev = xcdev->xdev;
	struct xdma2_engine *engine = xcdev->engine;
	int last_param;
	...
	xcdev->sys_device = device_create(g_xdma2_class, &xdev->pdev->dev,
...
static int destroy_xcdev(struct xdma2_cdev *cdev)
{
	if (!cdev) {
		pr_warn("cdev NULL.\n");
		return -EINVAL;
	}
	xdma2_debug_assert_msg(cdev->magic == MAGIC_CHAR,"char magic mismatch\n", -EINVAL);

	xdma2_debug_assert_ptr(cdev->xdev);

	xdma2_debug_assert_ptr(g_xdma2_class);

	xdma2_debug_assert_ptr(cdev->sys_device);

	if (cdev->sys_device)
		device_destroy(g_xdma2_class, cdev->cdevno);
...
static int create_xcdev(struct xdma2_pci_dev *xpdev, struct xdma2_cdev *xcdev,
			int bar, struct xdma2_engine *engine,
			enum cdev_type type)
{
	int rv;
	int minor;
	struct xdma2_dev *xdev = xpdev->xdev;
	dev_t dev;
	...
	if (!xpdev->major) {
		...
		int rv = alloc_chrdev_region(&dev, XDMA2_MINOR_BASE,
					XDMA2_MINOR_COUNT, XDMA2_NODE_NAME);
...
del_cdev:
	cdev_del(&xcdev->cdev);
unregister_region:
	unregister_chrdev_region(xcdev->cdevno, XDMA2_MINOR_COUNT);
	return rv;
}

void xpdev_destroy_interfaces(struct xdma2_pci_dev *xpdev)
{
	int i = 0;
	int rv;
#ifdef __XDMA2_SYSFS__
	device_remove_file(&xpdev->pdev->dev, &dev_attr_xdma2_dev_instance);
#endif
	...
	if (xpdev->major)
		unregister_chrdev_region(
				MKDEV(xpdev->major, XDMA2_MINOR_BASE),
				XDMA2_MINOR_COUNT);
}

int xpdev_create_interfaces(struct xdma2_pci_dev *xpdev)
{
	struct xdma2_dev *xdev = xpdev->xdev;
...
int xdma2_cdev_init(void)
{
#if KERNEL_VERSION_CHECK(9, 4, 6, 4, 0)
        g_xdma2_class = class_create(XDMA2_NODE_NAME);
#else
        g_xdma2_class = class_create(THIS_MODULE, XDMA2_NODE_NAME);
#endif
	if (IS_ERR(g_xdma2_class)) {
		dbg_init(XDMA2_NODE_NAME ": failed to create class");
		return -EINVAL;
	}
...
void xdma2_cdev_cleanup(void)
{
	

	if (g_xdma2_class)
		class_destroy(g_xdma2_class);
}
