...
static ssize_t char_ctrl_read(struct file *fp, char __user *buf, size_t count,
		loff_t *pos)
{
	struct xdma2_cdev *xcdev = (struct xdma2_cdev *)fp->private_data;
...
static long version_ioctl(struct xdma2_cdev *xcdev, void __user *arg)
{
	struct xdma2_ioc_info obj;
	struct xdma2_dev *xdev = xcdev->xdev;
	long rv;

	rv = copy_from_user((void *)&obj, arg, sizeof(struct xdma2_ioc_info));
	if (rv) {
		pr_err("copy from user failed %ld/%ld.\n",
			rv, sizeof(struct xdma2_ioc_info));
		return -EFAULT;
	}
	...
	if (copy_to_user(arg, &obj, sizeof(struct xdma2_ioc_info)))
		return -EFAULT;
	return 0;
}
...
long char_ctrl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct xdma2_cdev *xcdev = (struct xdma2_cdev *)filp->private_data;
	struct xdma2_dev *xdev;
	struct xdma2_ioc_base ioctl_obj;
	int rv;
	...
	xdma2_debug_assert_msg(_IOC_TYPE(cmd) == XDMA2_IOC_MAGIC, "bad magic.\n", -ENOTTY);
	...
	case XDMA2_IOCINFO:
		if (copy_from_user((void *)&ioctl_obj, (void __user *) arg,
			 sizeof(struct xdma2_ioc_base))) {
			pr_err("copy_from_user failed.\n");
			return -EFAULT;
		}

		xdma2_debug_assert_msg(ioctl_obj.magic == XDMA2_XCL_MAGIC,"magic !=  XDMA2_XCL_MAGIC.\n",	-ENOTTY);
		return version_ioctl(xcdev, (void __user *)arg);
	case XDMA2_IOCOFFLINE:
		xdma2_device_offline(xdev->pdev, xdev);
		break;
	case XDMA2_IOCONLINE:
		xdma2_device_online(xdev->pdev, xdev);
		break;
	default:
...
void cdev_ctrl_init(struct xdma2_cdev *xcdev)
{
