...
#define XDMA2_BAR_NUM						(6)

/* maximum amount of register space to map */
#define XDMA2_BAR_SIZE						(0x8000U)

#define XDMA2_CHANNEL_NUM_MAX				(4)

/*
 * interrupts per engine, rad2_vul.sv:237
 * .REG_IRQ_OUT	(reg_irq_from_ch[(channel*2) +: 2]),
 */
#define XDMA2_ENG_IRQ_NUM					(1)
#define XDMA2_PAGE_SIZE						0x1000
#define RX_STATUS_EOP						(1)

/* Target internal components on XDMA2 control BAR */
#define XDMA2_OFS_INT_CTRL					(0x2000U)
#define XDMA2_OFS_CONFIG					(0x3000U)

#define XDMA2_DESC_FIFO_DEPTH 512

/* maximum size of a single DMA transfer descriptor */
#define XDMA2_DESC_BLEN_BITS				28
#define XDMA2_DESC_BLEN_MAX					((1 << (XDMA2_DESC_BLEN_BITS)) - 1)

#define XDMA2_MAX_C2H_CREDITS				((1<<10)-1)
/* bits of the SG DMA control register */
#define XDMA2_CTRL_RUN_STOP					(1U << 0)
#define XDMA2_CTRL_IE_DESC_STOPPED			(1U << 1)
#define XDMA2_CTRL_IE_DESC_COMPLETED		(1U << 2)
#define XDMA2_CTRL_IE_DESC_ALIGN_MISMATCH	(1U << 3)
#define XDMA2_CTRL_IE_MAGIC_STOPPED			(1U << 4)
#define XDMA2_CTRL_IE_INVALID_LENGTH		(1U << 5)
#define XDMA2_CTRL_IE_IDLE_STOPPED			(1U << 6)
#define XDMA2_CTRL_IE_READ_ERROR			(0x1FU << 9)
#define XDMA2_CTRL_IE_WRITE_ERROR			(0x1FU << 14)
#define XDMA2_CTRL_IE_DESC_ERROR			(0x1FU << 19)
#define XDMA2_CTRL_NON_INCR_ADDR			(1U << 25)
#define XDMA2_CTRL_POLL_MODE_WB				(1U << 26)
#define XDMA2_CTRL_STM_MODE_WB				(1U << 27)

/* bits of the SG DMA status register */
#define XDMA2_STAT_BUSY						(1U << 0)
#define XDMA2_STAT_DESC_STOPPED				(1U << 1)
#define XDMA2_STAT_DESC_COMPLETED			(1U << 2)
#define XDMA2_STAT_ALIGN_MISMATCH			(1U << 3)
#define XDMA2_STAT_MAGIC_STOPPED			(1U << 4)
#define XDMA2_STAT_INVALID_LEN				(1U << 5)
#define XDMA2_STAT_IDLE_STOPPED				(1U << 6)

#define XDMA2_STAT_COMMON_ERR_MASK \
	(XDMA2_STAT_ALIGN_MISMATCH | XDMA2_STAT_MAGIC_STOPPED | \
	 XDMA2_STAT_INVALID_LEN)

/* desc_error, C2H & H2C */
#define XDMA2_STAT_DESC_UNSUPP_REQ			(1U << 19)
#define XDMA2_STAT_DESC_COMPL_ABORT			(1U << 20)
#define XDMA2_STAT_DESC_PARITY_ERR			(1U << 21)
#define XDMA2_STAT_DESC_HEADER_EP			(1U << 22)
#define XDMA2_STAT_DESC_UNEXP_COMPL			(1U << 23)

#define XDMA2_STAT_DESC_ERR_MASK	\
	(XDMA2_STAT_DESC_UNSUPP_REQ | XDMA2_STAT_DESC_COMPL_ABORT | \
	 XDMA2_STAT_DESC_PARITY_ERR | XDMA2_STAT_DESC_HEADER_EP | \
	 XDMA2_STAT_DESC_UNEXP_COMPL)

/* read error: H2C */
#define XDMA2_STAT_H2C_R_UNSUPP_REQ			(1U << 9)
#define XDMA2_STAT_H2C_R_COMPL_ABORT		(1U << 10)
#define XDMA2_STAT_H2C_R_PARITY_ERR			(1U << 11)
#define XDMA2_STAT_H2C_R_HEADER_EP			(1U << 12)
#define XDMA2_STAT_H2C_R_UNEXP_COMPL		(1U << 13)

#define XDMA2_STAT_H2C_R_ERR_MASK	\
	(XDMA2_STAT_H2C_R_UNSUPP_REQ | XDMA2_STAT_H2C_R_COMPL_ABORT | \
	 XDMA2_STAT_H2C_R_PARITY_ERR | XDMA2_STAT_H2C_R_HEADER_EP | \
	 XDMA2_STAT_H2C_R_UNEXP_COMPL)

/* write error, H2C only */
#define XDMA2_STAT_H2C_W_DECODE_ERR			(1U << 14)
#define XDMA2_STAT_H2C_W_SLAVE_ERR			(1U << 15)

#define XDMA2_STAT_H2C_W_ERR_MASK	\
	(XDMA2_STAT_H2C_W_DECODE_ERR | XDMA2_STAT_H2C_W_SLAVE_ERR)

/* read error: C2H */
#define XDMA2_STAT_C2H_R_DECODE_ERR			(1U << 9)
#define XDMA2_STAT_C2H_R_SLAVE_ERR			(1U << 10)

#define XDMA2_STAT_C2H_R_ERR_MASK	\
	(XDMA2_STAT_C2H_R_DECODE_ERR | XDMA2_STAT_C2H_R_SLAVE_ERR)

/* all combined */
#define XDMA2_STAT_H2C_ERR_MASK	\
	(XDMA2_STAT_COMMON_ERR_MASK | XDMA2_STAT_DESC_ERR_MASK | \
	 XDMA2_STAT_H2C_R_ERR_MASK | XDMA2_STAT_H2C_W_ERR_MASK)

#define XDMA2_STAT_C2H_ERR_MASK	\
	(XDMA2_STAT_COMMON_ERR_MASK | XDMA2_STAT_DESC_ERR_MASK | \
	 XDMA2_STAT_C2H_R_ERR_MASK)

/* bits of the SGDMA descriptor control field */
#define XDMA2_DESC_STOPPED					(1U << 0)
#define XDMA2_DESC_COMPLETED				(1U << 1)
#define XDMA2_DESC_EOP						(1U << 4)

#define XDMA2_PERF_RUN						(1U << 0)
#define XDMA2_PERF_CLEAR					(1U << 1)
#define XDMA2_PERF_AUTO						(1U << 2)

#define MAGIC_ENGINE						0xEEEEEEEEU
#define MAGIC_DEVICE						0xDDDDDDDDU

/* upper 16-bits of engine identifier register */
#define XDMA2_ID_H2C						0x1fc0U
#define XDMA2_ID_C2H						0x1fc1U
...
#define MAX_NUM_ENGINES						(XDMA2_CHANNEL_NUM_MAX * 2)
...
#define xdma2_debug_assert_msg(cond, msg, ret_val) \
		if (!(cond)){\
			pr_err("Debug assertion %s failed. %s", #cond, msg); \
			return (ret_val);\
			}
#define xdma2_debug_assert_ptr(ptr) xdma2_debug_assert_msg(ptr!=NULL, "Pointer " #ptr" is NULL.", -EINVAL)
#else
/* disable debugging */
#define dbg_io(...)
#define dbg_fops(...)
...
#define xdma2_debug_assert_msg(...)
#define xdma2_debug_assert_ptr(...)
#endif
...
struct xdma2_transfer {
	struct page **pages;
	unsigned int num_pages;
	struct sg_table sgt;
	generic_dma_record(struct xdma2_desc) *adj_desc_blocks;/*bookkeeping for descriptors grouped in adjacent blocks*/
...
struct xdma2_engine {
	...
	struct xdma2_dev *xdev;	/* parent device */
	...
	struct xdma2_transfer_params transfer_params;
	struct xdma2_transfer transfer;
	...
	generic_dma_record(volatile struct xdma2_poll_wb) poll_mode_wb;
#endif
	...
	struct xdma2_performance_ioctl xdma2_perf;	/* perf test control */
};

struct xdma2_user_irq {
	struct xdma2_dev *xdev;		/* parent device */
	u8 user_idx;			/* 0 ~ 15 */
...
struct xdma2_dev {
	...
	/* PCIe BAR management */
	void __iomem *bar[XDMA2_BAR_NUM];	/* addresses for mapped BARs */
	resource_size_t bar_size[XDMA2_BAR_NUM];		/* mapped size of BARs	*/ 
	int user_bar_idx;	/* BAR index of user logic */
	int config_bar_idx;	/* BAR index of XDMA2 config logic */
	int bypass_bar_idx;	/* BAR index of XDMA2 bypass logic */
	...
	struct xdma2_user_irq user_irq[16];	/* user IRQ management */
	unsigned int mask_irq_user;

	/* XDMA2 engine management */
	int engines_num;	/* Total engine count */
	u32 mask_irq_h2c;
	u32 mask_irq_c2h;
	struct xdma2_engine *engine_h2c;
	struct xdma2_engine *engine_c2h;
...
void xdma2_device_offline(struct pci_dev *pdev, void *dev_handle);
void xdma2_device_online(struct pci_dev *pdev, void *dev_handle);
ssize_t xdma2_xfer_submit(struct xdma2_engine *engine);
int xdma2_performance_submit(struct xdma2_engine *engine);
struct xdma2_transfer *engine_cyclic_stop(struct xdma2_engine *engine);
